...
Usage:
    python include/extraction/load_to_snowflake.py              # Load all sources
    python include/extraction/load_to_snowflake.py hn           # Load HN only
    python include/extraction/load_to_snowflake.py github       # Load GitHub only
    python include/extraction/load_to_snowflake.py linkedin     # Load LinkedIn only
    python include/extraction/load_to_snowflake.py verify       # Verify row counts
...
def load_hn_data():
    ...
    if not os.path.exists(parquet_path):
        ...
        print("  Run: python include/extraction/fetch_hn_data.py")
        return
...
def load_github_data():
    ...
    if not os.path.exists(json_path):
        ...
        print("  Run: python include/extraction/fetch_github_data.py")
        return
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if source == "hn":
            ...
            print("Usage: python include/extraction/load_to_snowflake.py [hn|github|linkedin|verify]")
    else:
        run_load_all()
